...
int main(int argc, char *argv[]) {
  ...
  if (args.stu_folder != "") {
      ...
      for (auto& sub_dir : sub_dirs) {
          ...
          if (bin_file != "" && asm_file != "") {
			  ofs << "Warning: both .bin and .asm file exist, using .bin file" << std::endl;
		  }
          // compile
		  std::string obj_filename;
		  if (bin_file != "") {
			  converter.convertBin(bin_file);
			  obj_filename = std::string(bin_file.substr(0, bin_file.find_last_of('.')) + ".obj");
		  }
		  else {
			  assembler.assemble(asm_file);
			  obj_filename = std::string(asm_file.substr(0, asm_file.find_last_of('.')) + ".obj");
		  }
